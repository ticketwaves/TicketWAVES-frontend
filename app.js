...
const __adminLoaded=new Set();
function adminTab(name,btn){document.querySelectorAll('.admin-section').forEach(x=>x.classList.remove('active'));$('admin'+name.charAt(0).toUpperCase()+name.slice(1)).classList.add('active');document.querySelectorAll('.admin-tabs button').forEach(x=>x.classList.remove('active'));if(btn)btn.classList.add('active');const fn={dash:loadAdminDash,users:loadAdminUsers,events:loadAdminEvents,tickets:loadAdminTickets,orders:loadAdminOrders,sell:loadAdminSell,giveaways:loadAdminGiveaways}[name];if(fn&&!__adminLoaded.has(name)){__adminLoaded.add(name);fn();}}
