...
async function api(path,opt={}){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),Number(opt.timeout||25000));try{const {timeout:_,...fetchOpt}=opt;const r=await fetch(API+path,{...fetchOpt,cache:'no-store',signal:controller.signal,headers:{...headers(),...(fetchOpt.headers||{})}});const txt=await r.text();let d={};try{d=txt?JSON.parse(txt):{}}catch{d={message:txt||'The server returned an invalid response.'}}if(!r.ok)throw Error(d.message||'Something went wrong.');return d}catch(e){if(e.name==='AbortError')throw Error('The TicketWAVES server took too long to respond. Please try again.');if(e instanceof TypeError)throw Error('Unable to reach the TicketWAVES server. Please check your connection and try again.');throw e}finally{clearTimeout(timeout)}}
...
async function login(e){e.preventDefault();const btn=e.submitter||e.target.querySelector('button[type="submit"],button.primary');setBusy(btn,true,'Signing in');try{const email=$('loginEmail')?.value.trim().toLowerCase(),password=$('loginPassword')?.value||'';if(!email||!password)throw Error('Enter your email and password.');const d=await api('/auth/login',{method:'POST',body:JSON.stringify({email,password}),timeout:25000});if(!d.token||!d.user)throw Error('The server returned an incomplete sign-in response.');token=d.token;user=d.user;localStorage.setItem('ticketwaves_token',token);updateAdminVisibility();if($('avatarBtn'))$('avatarBtn').textContent=(user.firstName||user.name||'☺').charAt(0).toUpperCase();toast('Welcome back! 💙');const after=localStorage.getItem('ticketwaves_after_transfer');if(after){localStorage.removeItem('ticketwaves_after_transfer');location.href=after;return}const pending=localStorage.getItem('ticketwaves_pending_transfer_token');if(pending){location.href='accept-transfer.html?token='+encodeURIComponent(pending);return}go(user.role==='admin'?'admin':'account')}catch(err){toast(err.message||'Sign in failed.')}finally{setBusy(btn,false)}}
...
async function forgot(e){e.preventDefault();const btn=$('forgotSubmit');setBusy(btn,true,'Sending');try{const d=await api('/auth/forgot-password',{method:'POST',body:JSON.stringify({email:$('forgotEmail').value.trim()}),timeout:25000});closeSheet();toast(d.message)}catch(err){toast(err.message)}finally{setBusy(btn,false)}}
...
function renderAdmin(){if(user?.role!=='admin'){toast('Administrator access required.');return}const b=$('adminBox');b.innerHTML=`<div class="page-title"><small>CONTROL ROOM</small><h1>Admin dashboard 🛠️</h1><p>Manage TicketWAVES without leaving the mobile experience.</p></div><div class="admin-tabs"><button class="active" onclick="adminTab('dash',this)">Overview</button><button onclick="adminTab('users',this)">Users</button><button onclick="adminTab('events',this)">Events</button><button onclick="adminTab('tickets',this)">Tickets</button><button onclick="adminTab('orders',this)">Orders</button><button onclick="adminTab('sell',this)">Sell requests</button><button onclick="adminTab('giveaways',this)">Verified Transfer</button></div><div id="adminDash" class="admin-section active"></div><div id="adminUsers" class="admin-section"></div><div id="adminEvents" class="admin-section"></div><div id="adminTickets" class="admin-section"></div><div id="adminOrders" class="admin-section"></div><div id="adminSell" class="admin-section"></div><div id="adminGiveaways" class="admin-section"></div><button class="danger wide" style="margin-top:16px" onclick="logout()">Sign out</button>`;loadAdminDash()}
async function adminTab(name,btn){document.querySelectorAll('.admin-section').forEach(x=>x.classList.remove('active'));const section=$('admin'+name.charAt(0).toUpperCase()+name.slice(1));if(section)section.classList.add('active');document.querySelectorAll('.admin-tabs button').forEach(x=>x.classList.remove('active'));if(btn)btn.classList.add('active');if(section&&!window.__adminLoaded?.[name])section.innerHTML='<div class="empty">Loading…</div>';const fn={dash:loadAdminDash,users:loadAdminUsers,events:loadAdminEvents,tickets:loadAdminTickets,orders:loadAdminOrders,sell:loadAdminSell,giveaways:loadAdminGiveaways}[name];if(fn){await fn();window.__adminLoaded=window.__adminLoaded||{};window.__adminLoaded[name]=true}}
...
async function loadAdminGiveaways(){
  try{
    const d=await api('/admin/giveaways?limit=50&page=1');const xs=Array.isArray(d)?d:(d.items||[]);
    $('adminGiveaways').innerHTML=`
      <div class="form-card giveaway-create-card">
        <div class="sheet-kicker">VERIFIED TRANSFER CENTER</div>
        <h2>Verified TicketWAVES Transfer (TT) 🎟️</h2>
        <p class="muted">Add the event, artwork and seat details, then send a secure transfer email to the recipient. A new user will be guided through account creation before the ticket is claimed.</p>
        <form onsubmit="createAdminVerifiedTransfer(event)">
          <label>Event title<input id="gwTitle" required placeholder="BTS WORLD TOUR..."></label>
          <div class="two"><label>Artist<input id="gwArtist" required></label><label>Category<select id="gwCategory"><option>Verified TicketWAVES Transfer (TT)</option><option>Concert</option><option>Sports</option><option>Theatre</option></select></label></div>
          ...
          <label>Event image URL<input id="gwImage" type="url" placeholder="https://..." oninput="previewVerifiedTransferImage()"></label>
          <label>Or upload image<input id="gwImageFile" type="file" accept="image/jpeg,image/png,image/webp" onchange="previewVerifiedTransferImageFile(event)"></label>
          <div id="gwImagePreview" class="image-preview">Transfer image preview</div>
          ...
          <label>Recipient email<input id="gwEmail" type="email" required placeholder="recipient@example.com"></label>
          <label>Ticket source<select id="gwSource" onchange="toggleTicketSource()"><option>TicketWAVES</option><option>Ticketmaster</option><option>AXS</option><option>Viagogo</option><option>Ticketera</option><option>TicketSwap</option><option>Other</option></select></label>
          <input id="gwCustomSource" placeholder="Platform name" style="display:none">
          <label>Ticket note<textarea id="gwInfo" rows="3" placeholder="Optional ticket/transfer information"></textarea></label>
          <button id="gwSubmit" class="primary wide action-btn">Create & send transfer</button>
        </form>
      </div>
      <div class="section-head"><div><small>RECENT</small><h2>Verified Transfer history</h2></div></div>
      <div class="admin-list">${xs.map(g=>{
        const name=g.recipient?.name||g.recipient?.email||g.recipientEmail||'Pending';
        return `<div class="admin-row"><div><b>${esc(g.eventTitle||'Verified Transfer')}</b><br><small>${esc(g.date||'')} · ${esc(g.venue||'')}</small><br><small>Recipient: ${esc(name)} · Section ${esc(g.section||'General')} · Row ${esc(g.row||'—')} · Seat ${esc(g.seat||'—')}</small></div>${g.ticketId?`<button class="secondary action-btn" onclick="resendVerifiedTransfer('${esc(g.id)}')">Resend</button>`:''}</div>`;
      }).join('')||'<div class="empty">No verified transfers yet.</div>'}</div>`;
  }catch(e){$('adminGiveaways').innerHTML=`<div class="empty">${esc(e.message)}</div>`}
}
function previewVerifiedTransferImage(){
  const url=$('gwImage')?.value.trim(),p=$('gwImagePreview');if(!p)return;
  if(url){p.innerHTML=`<img src="${esc(url)}" alt="Transfer preview" onerror="this.parentElement.textContent='Image URL could not be loaded.'">`}else p.textContent='Transfer image preview';
}
function previewVerifiedTransferImageFile(ev){
  const file=ev.target.files?.[0],p=$('gwImagePreview');if(!file||!p)return;
  if(file.size>2.5*1024*1024){toast('Please choose an image smaller than 2.5 MB.');ev.target.value='';return}
  const reader=new FileReader();reader.onload=()=>{p.innerHTML=`<img src="${esc(reader.result)}" alt="Transfer preview">`;p.dataset.image=reader.result};reader.readAsDataURL(file);
}
function toggleTicketSource(){const s=$('gwSource'),c=$('gwCustomSource');if(!s||!c)return;c.style.display=s.value==='Other'?'block':'none';if(s.value!=='Other')c.value='';}
async function createAdminVerifiedTransfer(e){
  ...
  try{
    ...
    await api('/admin/giveaways',{method:'POST',body:JSON.stringify({
      ...
      ticketSource:$('gwSource').value,customTicketSource:$('gwCustomSource').value.trim(),additionalInfo:$('gwInfo').value.trim()
    })});
    toast('Verified TicketWAVES Transfer (TT) created and email queued. 🎉');loadAdminGiveaways();
  }catch(err){toast(err.message)}finally{setBusy(btn,false)}
}
async function resendVerifiedTransfer(id){
