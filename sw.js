const CACHE='ticketwaves-shell-v26-stable';
const SHELL=['./','./index.html','./accept-transfer.html','./payment-callback.html','./manifest.webmanifest','./config.js'];
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname.endsWith('/index.html') || u.pathname.endsWith('/config.js')){e.respondWith(fetch(e.request,{cache:'no-store'}).then(r=>{if(r.ok&&u.origin===location.origin){const copy=r.clone();caches.open(CACHE).then(c=>c.put(e.request,copy)).catch(()=>{});}return r}).catch(()=>caches.match(e.request)));return;}
